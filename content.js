...
(() => {
  ...
  function isCanadaSite() {
    return window.location.hostname === 'shop.lululemon.com' &&
           window.location.pathname.startsWith('/en-ca/');
  }
  ...
  function isIntlSite() {
    ...
    return (host.includes('lululemon.com') &&
           ...
           host.includes('lululemon.co.kr') ||
           host.includes('lululemon.co.uk') ||
           host.includes('lululemon.fr');
  }
  ...
  function getStockStatusFromDOM() {
    ...
    if (sfccAtb) {
      // Check if a size is actually selected first — SFCC sites keep the button
      // disabled when no size is picked, not because the product is sold out
      const sizeSelected = document.querySelector('input[name="select-size-US"]:checked, .swatch-attribute.size .swatch-option.selected, .product-size-selected, .size-swatch.selected');
      const isDisabled = sfccAtb.disabled || sfccAtb.classList.contains('disabled');
      if (isDisabled && !sizeSelected) {
        // No size selected — don't treat as sold out, just return stock status
        // from JSON-LD data instead
        return 'in_stock';
      }
      if (isDisabled) {
        return 'sold_out';
  ...
  function extractIntlProductData() {
    ...
    return {
      ...
      region: window.location.pathname.startsWith('/en-vn/') ? 'vn' :
              window.location.hostname.includes('.hk') ? 'hk' :
              window.location.hostname.includes('.au') ? 'au' :
              window.location.hostname.includes('.co.jp') ? 'jp' :
              window.location.hostname.includes('.co.kr') ? 'kr' :
              window.location.hostname.includes('.co.uk') ? 'uk' :
              window.location.hostname.includes('.fr') ? 'fr' : 'intl',
      lastChecked: Date.now(),
    };
  }
